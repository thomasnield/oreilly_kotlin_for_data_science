...
data class SocialUser(
    ...
    fun recommendedFriends() = users.asSequence()
            ...
            .map { otherUser -> // package up mutual friends
                MutalFriendships(this, otherUser, mutualFriendsOf(otherUser).toList())
            }.filter { it.mutualFriends.count() > 0 } // omit where no MutualFriendships exist
            .sortedByDescending { it.mutualFriends.count() } // sort
}

data class MutalFriendships(val user: SocialUser, val otherUser: SocialUser, val mutualFriends: List<SocialUser>)
